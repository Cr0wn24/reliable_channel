...
sending_small_reliable_packets :: proc(t: ^testing.T) {
  ...
  for {
    ...
    time.accurate_sleep(time.Millisecond*1)
...
@test
sending_small_reliable_packets_at_once :: proc(t: ^testing.T) {
  ...
  for i in 0..=send_count {
    rc.channel_send(channel0, mem.any_to_bytes(i), is_reliable = true)
    rc.channel_send(channel1, mem.any_to_bytes(i), is_reliable = true)
  }
